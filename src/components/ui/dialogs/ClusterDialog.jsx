...
const ClusterMemberRow = React.memo(function ClusterMemberRow({ m, isPrimary, tagOptions, onUpdateTag, openContextMenu, selectSize = 'sm' }) {
  ...
  const opt = React.useMemo(() => {
    ...
    const mergedDevices = uniq([...(DEFAULT_CLUSTER_TAG_OPTIONS.devices || []), ...(tagOptions.devices || [])]);
    const mergedRes = uniq([...(DEFAULT_CLUSTER_TAG_OPTIONS.resolutions || []), ...(tagOptions.resolutions || [])]);
    const mergedVer = uniq([...(DEFAULT_CLUSTER_TAG_OPTIONS.versions || []), ...(tagOptions.versions || [])]);
    return {
  ...
  return (
    ...
    >
      <div style={{ width: 44, height: 44, borderRadius: 8, overflow: 'hidden', background: 'var(--overlay-on-light-08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <img src={m.iconUrl || '/icon.ico'} alt="icon" style={{ width: 44, height: 44, objectFit: 'cover' }} draggable={false} />
      </div>
      <div title={m.gameName} style={{ fontWeight: 600, paddingRight: 8, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.gameName}</div>
      <div title={m.vendor} style={{ opacity: 0.85, paddingRight: 8, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.vendor || '-'}</div>
      <div>
        <Select
          size={selectSize}
...
export default function ClusterDialog({ isOpen, clusterId, onClose }) {
  ...
  const [tagOptions, setTagOptions] = React.useState({ devices: [], resolutions: [], versions: [] });
  ...
  const { ContextMenuElement, openContextMenu } = useContextMenu({
    ...
    onCreateShortcut: (m) => handleCreateShortcut(m.filePath),
  ...
  const loadData = React.useCallback(async (opts = {}) => {
      ...
      setMembers((prev) => {
        try {
          if (Array.isArray(prev) && prev.length === next.length) {
            ...
            const mapA = new Map(prev.map(x => [String(x.filePath || ''), JSON.stringify(x.tags || null)]));
            let same = true;
            for (const m of next) {
              const k = String(m.filePath || '');
              const v = JSON.stringify(m.tags || null);
              if (!mapA.has(k) || mapA.get(k) !== v) { same = false; break; }
            }
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const extras = (opts && typeof opts === 'object') ? {
    ...
    const off = window.electronAPI.onClusterChanged?.((payload) => {
      try {
        if (payload && payload.id && payload.id === clusterId) {
          ...
          if (deltaPrimary >= 0 && deltaPrimary < 1200 && payload.action && payload.action !== 'members-updated') {
            return;
          }
          // 3) 剛移除成員：若事件提示為結構變更（移除），忽略全量重載，交由本地狀態生效
          if (payload.action === 'members-updated' && deltaRemoval >= 0 && deltaRemoval < 1200 && (payload.removed && payload.removed > 0)) {
            return;
          }
          // 非本地觸發或超過時間窗：
          if (payload.action === 'members-updated') {
            const hasStructureChange = (payload && ((payload.added && payload.added > 0) || (payload.removed && payload.removed > 0)));
            ...
            if (hasStructureChange) {
              ...
              setMembers(prev => prev.map(x => String(x.filePath || '') === k ? { ...x, tags: nextTags } : x));
            } else {
  ...
  React.useEffect(() => {
    const off = window.electronAPI?.onGamesIncrementalUpdate?.((update) => {
      try {
        // 兼容多種回傳結構
        const arr = Array.isArray(update?.games) ? update.games : (Array.isArray(update) ? update : (update ? [update] : []));
  ...
  const handleUpdateTag = React.useCallback(async (filePath, key, value) => {
    try {
  ...
  const handleRemoveMember = async (filePath) => {
    try {
      ...
      setCluster(prev => {
        if (!prev) return prev;
        if ((prev.primaryFilePath || '').toLowerCase() !== (filePath || '').toLowerCase()) return prev;
  ...
  const Header = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{ fontSize: 14, opacity: 0.85 }}>
        {cluster ? `${(cluster.name || t('cluster.untitled'))} · ${members.length}` : t('app.loading')}
      </div>
    </div>
  );
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0, height: '100%' }}>
        ...
        <div style={{ flex: '1 1 auto', minHeight: 0, overflow: 'auto', paddingRight: 4 }}>
          {loading ? (
            ...
            const isPrimary = cluster && cluster.primaryFilePath && (cluster.primaryFilePath.toLowerCase() === (m.filePath || '').toLowerCase());
            return (
              <ClusterMemberRow
                key={m.filePath}
                m={m}
