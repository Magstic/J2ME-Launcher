...
const DROPBOX_APP_KEY = 'f2l61y65nwo0pex'; // TODO: 若您是開發者，請換成自己的 App Key
...
export default function BackupDialog({ isOpen, onClose }) {
          ...
          if (auth && auth.linked) {
            ...
            if (acct && acct.profile_photo_url) {
              try {
                ...
                if (mounted) setDropboxAvatar(null);
              }
            } else {
  ...
  const handleToggleGroup = (key) => (value) => {
    setSelectedGroups((prev) => {
      ...
      try {
        localStorage.setItem('backup.selectedGroups', JSON.stringify(next));
      } catch (_) {}
      return next;
    });
  };
  ...
  const footer = (
    <>
      ...
      <div className="flex gap-8 push-right">
        <button className="btn soft-hover" disabled={busy} onClick={() => runRestore()}>
          {t('sync.restore')}
        </button>
        <button className="btn btn-secondary" disabled={busy} onClick={() => runBackup('full')}>
          {t('sync.full')}
        </button>
        <button
    ...
    try {
      ...
      if (typeof res?.uploaded === 'number') {
        setProgress((prev) => ({
          ...
          total: Math.max(prev.total || 0, res.uploaded),
          phase: 'finalizing',
        }));
      }
    } catch (e) {
      ...
      setTimeout(
        () => setProgress((p) => (p.done > 0 || p.total > 0 ? { ...p, phase: 'done' } : p)),
        50
      );
    }
  }
  ...
  async function runRestore() {
    ...
    try {
      const groups = Object.entries(selectedGroups)
        .filter(([, v]) => v)
        .map(([k]) => k);
      const plan = await api.backupRestorePlan({
        provider,
      ...
      setInfoDialog({
        ...
        message: t('sync.restoreFailed') + (e && e.message ? e.message : String(e)),
      });
    } finally {
      setBusy(false);
      setTimeout(
    ...
    if (provider === 's3') {
      ...
      const setP = (k, v) => {
        setParams((prev) => {
          ...
          try {
            localStorage.setItem('backup.s3.params', JSON.stringify(next));
          } catch (_) {}
          try {
            api && api.backupSetProviderParams && api.backupSetProviderParams('s3', next);
          } catch (_) {}
          return { ...prev, s3: next };
        });
      };
        ...
        <div className="card card-muted p-12">
          ...
          <div className="form-row">
            ...
            <input
              ...
              type="text"
              value={p.bucket || ''}
              onChange={(e) => setP('bucket', e.target.value)}
          ...
          <div className="form-row">
            ...
            <input
              ...
              type="text"
              value={p.accessKeyId || ''}
              onChange={(e) => setP('accessKeyId', e.target.value)}
          ...
          <div className="form-row">
            ...
            <input
              ...
              onChange={(e) => setP('secretAccessKey', e.target.value)}
          ...
          <div className="form-row">
            ...
            <input
              ...
              type="text"
              value={p.prefix || ''}
              onChange={(e) => setP('prefix', e.target.value)}
              placeholder={t('sync.s3.prefix')}
            />
          </div>
          <div className="form-row">
            <ToggleSwitch
              checked={p.forcePathStyle ?? true}
              onChange={(v) => setP('forcePathStyle', !!v)}
    ...
    if (provider === 'dropbox') {
      ...
      const setP = (k, v) => {
        setParams((prev) => {
          ...
          try {
            localStorage.setItem('backup.dropbox.params', JSON.stringify(next));
          } catch (_) {}
      ...
      const handleConnect = async () => {
        ...
        setInfoDialog({
          ...
          title: 'Dropbox',
          message:
            t('sync.dropbox.linkedFailedMessage') +
      ...
      return (
        <div className="card card-muted p-12">
          ...
          <div
            style={{
              border: '1px solid var(--overlay-on-light-16)',
              borderRadius: 8,
              padding: 12,
              marginTop: 12,
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              {dropboxAvatar ? (
                ...
                >
                  {dropboxAccount && dropboxAccount.name && dropboxAccount.name.display_name
                    ? dropboxAccount.name.display_name.slice(0, 1).toUpperCase()
                    : 'D'}
                </div>
              )}
              {/* Info */}
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  {dropboxAccount && dropboxAccount.name
                    ? dropboxAccount.name.display_name || t('sync.dropbox.nameless')
                    : t('sync.dropbox.nameless')}
                </div>
                <div
                  style={{
                    opacity: 0.8,
                ...
                >
                  {dropboxAccount && dropboxAccount.email
                    ? dropboxAccount.email
                    : dropboxAuth?.linked
                      ? '—'
                      : t('sync.dropbox.notLinked')}
                </div>
              </div>
              {/* Action */}
              {dropboxAuth?.linked ? (
                <button className="btn btn-secondary" onClick={handleUnlink}>
                  {t('sync.dropbox.unlink')}
                </button>
              ) : (
                <button className="btn btn-primary" onClick={handleConnect}>
                  {t('sync.dropbox.connect')}
                </button>
              )}
            </div>
          </div>
          <div className="form-row">
            ...
            <input
              ...
              value={p.prefix || ''}
              onChange={(e) => setP('prefix', e.target.value)}
              placeholder={t('sync.dropbox.prefix')}
            />
          </div>
        </div>
      );
    ...
    if (provider === 'webdav') {
      ...
      const setP = (k, v) => {
        setParams((prev) => {
          ...
          try {
            localStorage.setItem('backup.webdav.params', JSON.stringify(next));
          } catch (_) {}
          try {
            api && api.backupSetProviderParams && api.backupSetProviderParams('webdav', next);
          } catch (_) {}
        ...
        <div className="card card-muted p-12">
          ...
          <div className="form-row">
            ...
            <input
              ...
              value={p.prefix || ''}
              onChange={(e) => setP('prefix', e.target.value)}
              placeholder={t('sync.webdav.prefix')}
            />
          </div>
          <div className="form-row">
            ...
            <input
              ...
              placeholder={t('sync.webdav.username')}
            />
          </div>
          <div className="form-row">
            ...
            <input
              ...
              placeholder={t('sync.webdav.password')}
            />
          </div>
          <div className="form-row">
            ...
            <input
              ...
              type="password"
              value={p.bearerToken || ''}
              onChange={(e) => setP('bearerToken', e.target.value)}
              placeholder={t('sync.webdav.bearerToken')}
            />
          </div>
  ...
  return (
    <>
      ...
      >
        ...
        <div className="card card-muted p-12 mb-12">
          <div className="form-row">
            ...
            <Select options={PROVIDERS} value={provider} onChange={(v) => setProvider(v)} />
          </div>
        </div>

        {/* 服務參數（依 provider 動態出現欄位，先佔位框架） */}
        <div className="mb-12">{renderProviderParams()}</div>
      ...
      <ConflictResolveDialog
        ...
        onProceed={async (opts) => {
          await handleConflictProceed(opts);
        }}
        onClose={() => {
          setConflictOpen(false);
          setConflictPlan(null);
