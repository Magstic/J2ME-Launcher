...
export default function useDragSession({ selectedRef, games, source = { type: 'desktop', id: null }, clusters = [], selectedClustersRef = { current: new Set() } }) {
  ...
  const handleGameDragStart = React.useCallback((event, game) => {
    ...
    // 準備多選項目（優先簇）
    const originallySelected = !!(selectedRef.current && selectedRef.current.has && selectedRef.current.has(game.filePath));
    ...
    for (const c of clusters) { if (c && c.id) clusterMap.set(c.id, c); }
    const clusterItems = isDraggingSelectedGame && selectedClusterIds.size > 0
      ? Array.from(selectedClusterIds).map(cid => {
      ...
      : [];
    const gameItems = (games || []).filter(g => currentGameSel.has(g.filePath)).map(g => ({
      gameId: g.filePath,
      filePath: g.filePath,
      type: 'game',
      name: g.gameName,
    ...
    try { console.log('[DRAG_UI] dragStart(game) for', game && game.filePath, 'gamesSel=', currentGameSel.size, 'clustersSel=', selectedClusterIds.size, 'items.len=', items.length, 'draggingSelected=', isDraggingSelectedGame, 'source=', source); } catch {}
  ...
  const handleClusterDragStart = React.useCallback((event, cluster) => {
    event.dataTransfer.effectAllowed = 'move';
    try { event.dataTransfer.setData('text/plain', cluster?.name || 'Cluster'); } catch (_) {}
    try { event.dataTransfer.setData('application/x-j2me-internal', '1'); } catch (_) {}
    ...
    const items = [...clusterItems, ...gameItems];
    try { console.log('[DRAG_UI] cluster dragStart for', cluster && cluster.id, 'clustersSel=', currentClusterSel.size, 'gamesSel=', currentGameSel.size, 'items.len=', items.length, 'source=', source); } catch {}

    // 啟動跨窗口拖拽會話（多選簇）
    try {
      const ctx = source || { type: 'desktop', id: null };
      const p = window.electronAPI?.startDragSession?.(items, ctx);
      try { console.log('[DRAG_UI] startDragSession (clusters) invoked with items=', items.length, 'source=', ctx); } catch {}
      try { p && p.then?.((res) => { try { console.log('[DRAG_UI] startDragSession (clusters) result:', res); } catch {} })?.catch?.((err) => { try { console.warn('[DRAG_UI] startDragSession (clusters) error:', err && err.message); } catch {} }); } catch {}
    } catch (e) {
