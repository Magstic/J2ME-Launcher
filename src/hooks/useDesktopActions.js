...
export const useDesktopActions = ({
  ...
  // 將遊戲添加到資料夾
  const handleGameDrop = useCallback(
    async (gameFilePath, folderId) => {
      try {
  ...
  const handleFolderSelect = useCallback(
    async (selectedFolder, folderSelectDialog) => {
      ...
      const uniqueGames =
        Array.isArray(fileList) && fileList.length > 0
          ...
          : target && target.type === 'game' && target.filePath
            ? [String(target.filePath)]
            : [];

      let uniqueClusters =
        Array.isArray(clusterIdsFromDialog) && clusterIdsFromDialog.length > 0
      ...
      try {
        ...
        const folderIdStr =
          typeof selectedFolder.id === 'string'
            ? selectedFolder.id
        ...
        if (uniqueGames.length > 0) {
          if (window.electronAPI?.batchAddGamesToFolder) {
            await window.electronAPI.batchAddGamesToFolder(uniqueGames, folderIdStr, {
              threshold: 30,
              chunkSize: 50,
              quiet: true,
            });
            ...
            await window.electronAPI.addGamesToFolderBatch(uniqueGames, folderIdStr, {
              quiet: true,
            });
            done += uniqueGames.length;
            setBulkStatus((prev) => ({ ...prev, done }));
        ...
        const errorMessage =
          typeof error?.message === 'string'
            ? error.message
            : String(error || t('desktopManager.common.unknownError'));
        const dialogTitle = String(t('desktopManager.addToFolder.title') || '加入資料夾');
        const dialogMessage = String(
          t('desktopManager.info.addToFolder.failWithReason', { reason: errorMessage }) ||
            `加入資料夾失敗: ${errorMessage}`
        );
  ...
  const handleRemoveGameFromFolder = useCallback(
    async (game, folderId) => {
      try {
        if (window.electronAPI?.removeGameFromFolder) {
          ...
          if (result.success) {
            ...
            setInfoDialog({
              isOpen: true,
              title: t('desktopManager.removeFromFolder.title'),
              message: t('desktopManager.info.removeFromFolder.fail'),
            });
          }
        }
      } catch (error) {
        ...
        setInfoDialog({
          isOpen: true,
          title: t('desktopManager.removeFromFolder.title'),
          message: t('desktopManager.info.removeFromFolder.failWithReason', {
  ...
  // 處理跨窗口遊戲移動
  const handleMoveGameBetweenFolders = useCallback(
    async (game, fromFolderId, toFolderId) => {
      try {
        if (window.electronAPI?.moveGameBetweenFolders) {
          const result = await window.electronAPI.moveGameBetweenFolders(
            game.filePath,
            fromFolderId,
            toFolderId
          );
          if (result.success) {
            ...
            setInfoDialog({
              isOpen: true,
              title: t('desktopManager.moveGame.title'),
              message: t('desktopManager.info.moveGame.fail'),
            });
          }
        }
      } catch (error) {
        ...
        setInfoDialog({
          isOpen: true,
          title: t('desktopManager.moveGame.title'),
          message: t('desktopManager.info.moveGame.failWithReason', {
            reason: error?.message || t('desktopManager.common.unknownError'),
          }),
        });
      }
