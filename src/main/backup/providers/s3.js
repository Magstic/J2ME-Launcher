...
function createS3Provider(params = {}) {
  ...
  return {
    async readText(relPath) {
      ...
      try {
        try {
          console.log('[backup:s3] readText', { Key });
        } catch (_) {}
        ...
        try {
          console.log('[backup:s3] readText error', { Key, error: e && e.message });
        } catch (_) {}
        return null;
      }
    },
    async writeText(relPath, text) {
      ...
      try {
        console.log('[backup:s3] writeText', { Key, bytes: Body.length });
      } catch (_) {}
    ...
    async uploadFile(relPath, localPath, size, onProgress) {
      ...
      try {
        console.log('[backup:s3] uploadFile start', { Key, localPath, size });
      } catch (_) {}
      ...
      await upload.done();
      try {
        console.log('[backup:s3] uploadFile done', { Key });
      } catch (_) {}
    },
    async deleteFile(relPath) {
      const Key = toKey(relPath);
      try {
        console.log('[backup:s3] deleteFile', { Key });
      } catch (_) {}
      try {
        await client.send(new DeleteObjectCommand({ Bucket: bucket, Key }));
      } catch (e) {
        try {
          console.log('[backup:s3] deleteFile error', { Key, error: e && e.message });
        } catch (_) {}
      }
    },
    async downloadFile(relPath, localPath) {
      ...
      try {
        console.log('[backup:s3] downloadFile start', { Key, localPath });
      } catch (_) {}
      const out = await client.send(new GetObjectCommand({ Bucket: bucket, Key }));
      ...
      try {
        console.log('[backup:s3] downloadFile done', { Key });
      } catch (_) {}
    },
    async list(prefixRel = '') {
      ...
      try {
        try {
          console.log('[backup:s3] list', { Prefix });
        } catch (_) {}
        ...
        try {
          console.log('[backup:s3] list error', { Prefix, error: e && e.message });
        } catch (_) {}
        return [];
      }
    },
