...
function getGamesMinimal() {
  const stmt = getPreparedStatement('games_minimal', `
...
function getGamesPaginated(offset = 0, limit = 1000) {
  const stmt = getPreparedStatement('games_paginated', `
    SELECT filePath, gameName, vendor, version, iconPath, md5
    FROM games g
    WHERE EXISTS (
      SELECT 1 FROM directories d
...
function getGamesByPaths(filePaths) {
  ...
  const stmt = getPreparedStatement(`games_by_paths_${filePaths.length}`, `
...
function getGamesCount() {
  const stmt = getPreparedStatement('games_count', `
...
function getFolderMembershipBatch(filePaths) {
  ...
  const placeholders = filePaths.map(() => '?').join(',');
  const stmt = getPreparedStatement(`folder_membership_${filePaths.length}`, `
