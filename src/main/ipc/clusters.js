...
function register({ ipcMain, DataStore, addUrlToGames, toIconUrl, broadcastToAll }) {
  ...
  ipcMain.handle('clusters:create', (event, payload) => {
    try {
      ...
      return { success: false, error: unique ? 'E_UNIQUE_MEMBER' : 'E_CREATE_FAILED', detail: unique ? msg.slice('E_UNIQUE_MEMBER: '.length) : msg };
    }
  });
  ...
  ipcMain.handle('clusters:merge', (event, fromId, toId) => {
    try {
      ...
      try {
        const db = getDB();
        const rows = db.prepare(`SELECT folderId FROM folder_clusters WHERE clusterId = ?`).all(toId);
        const folderIds = rows.map(r => r.folderId);
        if (folderIds.length > 0) {
          const { getFolderById } = require('../sql/folders-read');
          for (const fid of folderIds) {
            try { const f = getFolderById(fid); if (f) broadcastToAll('folder-updated', f); } catch (_) {}
          }
        }
      } catch (_) {}
      return res;
  ...
  ipcMain.handle('clusters:update-member-tags', (event, clusterId, filePath, tags) => {
    try {
      ...
      broadcastToAll('cluster:changed', { id: clusterId, action: 'members-updated', filePath: fp, tags });
      return res;
    } catch (error) {
  ...
  ipcMain.handle('clusters:add-games', (event, clusterId, filePaths) => {
    try {
      ...
      broadcastToAll('cluster:changed', { id: clusterId, action: 'members-updated', added: res.added });
      // 若該簇屬於某些資料夾，則這些資料夾的遊戲數需包含簇成員，廣播更新以刷新角標
      try {
        const db = getDB();
        const rows = db.prepare(`SELECT folderId FROM folder_clusters WHERE clusterId = ?`).all(clusterId);
        const folderIds = rows.map(r => r.folderId);
        if (folderIds.length > 0) {
          const { getFolderById } = require('../sql/folders-read');
          for (const fid of folderIds) {
      ...
      const msg = String(error && error.message ? error.message : error);
      const unique = msg.startsWith('E_UNIQUE_MEMBER: ');
      return { success: false, error: unique ? 'E_UNIQUE_MEMBER' : 'E_ADD_FAILED', detail: unique ? msg.slice('E_UNIQUE_MEMBER: '.length) : msg };
    }
  });
  ...
  ipcMain.handle('clusters:remove-game', (event, clusterId, filePath) => {
    try {
      ...
      broadcastToAll('cluster:changed', { id: clusterId, action: 'members-updated', removed: res.removed });
      // 同步刷新包含該簇的所有資料夾的角標
      try {
        const db = getDB();
        const rows = db.prepare(`SELECT folderId FROM folder_clusters WHERE clusterId = ?`).all(clusterId);
  ...
  ipcMain.handle('clusters:delete', (event, id) => {
    try {
      ...
      const folderRows = db.prepare(`SELECT folderId FROM folder_clusters WHERE clusterId = ?`).all(id);
      ...
      if (folderIds.length > 0 && memberPaths.length > 0) {
        try {
          ...
          try { console.warn('[clusters:delete] release to folders failed:', e?.message || e); } catch (_) {}
        }
      }
  ...
  ipcMain.handle('clusters:remove-from-folder', (event, clusterId, folderId) => {
    try {
      ...
      // 通知資料夾內容可能變更（簇已移除）
      try { broadcastToAll('folder-changed'); } catch (_) {}
  ...
  ipcMain.handle('clusters:cleanup-orphans', () => {
    try {
      ...
      try { broadcastToAll('cluster:changed', { action: 'cleanup-orphans', deleted: res.deleted }); } catch (_) {}
      return res;
    } catch (error) {
      return { success: false, error: String(error && error.message ? error.message : error) };
    }
