...
function getProviderAuthPath(app, provider) {
  ...
  if (provider === 'webdav')
    return path.join(app.getPath('userData'), META_DIR, AUTH_DIR, WEBDAV_AUTH);
  return null;
}
...
function register({ ipcMain, app }) {
  ...
  ipcMain.handle('backup:get-last', async () => {
    ...
    try {
      console.log('[backup:get-last]', meta);
    } catch (_) {}
    return meta;
  });
  ...
  ipcMain.handle('backup:set-provider-params', async (_e, { provider, params }) => {
    ...
    try {
      ...
      try {
        console.log('[backup:set-provider-params] error', e && e.message);
      } catch (_) {}
      return { ok: false };
    }
  });
  ...
  ipcMain.handle('dropbox:get-auth', async () => {
    ...
    return {
      linked: !!(auth && auth.accessToken),
      clientId: auth && auth.clientId ? auth.clientId : null,
    };
  });
  ...
  ipcMain.handle('dropbox:get-account', async () => {
    ...
    if (res.status === 401 && auth.refreshToken && auth.clientId) {
      ...
      try {
        ...
        const tokenRes = await httpPostToken(
        ...
        if (tokenRes.status >= 200 && tokenRes.status < 300) {
          ...
          try {
            console.log('[dropbox:get-account] refresh fail', tokenRes.status, tokenRes.text);
          } catch (_) {}
        }
      } catch (e) {
        try {
          console.log('[dropbox:get-account] refresh error', e && e.message);
        } catch (_) {}
      }
    }
    if (res.status < 200 || res.status >= 300) {
      try {
        console.log('[dropbox:get-account] http', res.status, res.text);
      } catch (_) {}
      return null;
    }
    try {
      ...
      try {
        console.log('[dropbox:get-account] parse error', e && e.message);
      } catch (_) {}
      return null;
    }
  });
  ...
  ipcMain.handle('dropbox:get-account-photo', async (_e, urlStr) => {
    try {
      ...
      try {
        console.log('[dropbox:get-account-photo] error', e && e.message);
      } catch (_) {}
      return null;
    }
  });
    ...
    const result = await new Promise((resolve, reject) => {
      server = http.createServer(async (req, res) => {
        try {
          ...
          if (u.pathname !== '/dropbox/callback') {
            res.statusCode = 404;
            res.end('Not Found');
            return;
          }
          ...
          if (tokenRes.status < 200 || tokenRes.status >= 300) {
            try {
              console.log('[dropbox:oauth] token error', tokenRes.status, tokenRes.text);
            } catch (_) {}
            res.statusCode = 500;
            res.end('Token exchange failed. You can close this window.');
          ...
          if (!json || !json.access_token) {
            res.statusCode = 500;
            res.end('Invalid token response. You can close this window.');
          ...
          try {
            console.log('[dropbox:oauth] error', e && e.message);
          } catch (_) {}
          res.statusCode = 500;
          res.end('Internal error. You can close this window.');
      ...
      server.on('error', (e) => {
        try {
          console.log('[dropbox:oauth] server error', e && e.message);
        } catch (_) {}
        reject(e);
      });
    });
  ...
  ipcMain.handle('backup:run', async (e, payload) => {
    ...
    try {
      console.log('[backup:run] start', { mode, provider, params: redacted, groups });
    } catch (_) {}
    ...
    writeMeta(app, meta);
    try {
      console.log('[backup:run] done', { ...result, lastBackupAt: meta.lastBackupAt });
    } catch (_) {}
  ...
  ipcMain.handle('backup:restore-plan', async (_e, payload) => {
    ...
    if (redacted.secretAccessKey) redacted.secretAccessKey = '***';
    try {
      console.log('[restore:plan] start', { provider, params: redacted, groups });
    } catch (_) {}
    let providerParams = { ...params };
    if (provider === 'dropbox') {
      const auth = readDropboxAuth(app);
    ...
    try {
      console.log('[restore:plan] done', res);
    } catch (_) {}
    return res;
  });
  ...
  ipcMain.handle('backup:restore-run', async (_e, payload) => {
    ...
    if (redacted.secretAccessKey) redacted.secretAccessKey = '***';
    try {
      console.log('[restore:run] start', {
        provider,
        params: redacted,
        groups,
        force,
        includePaths: Array.isArray(includePaths) ? includePaths.length : null,
      });
    } catch (_) {}
    let providerParams = { ...params };
    if (provider === 'dropbox') {
    ...
    try {
      console.log('[restore:run] done', res);
    } catch (_) {}
    return res;
  });
}
