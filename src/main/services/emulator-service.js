...
const { quoteArg, buildCommandLine } = require('../utils/java.js');
  ...
  async function launchGame(gameFilePath) {
    try {
      ...
      if (selectedEmulator === 'ke') {
        ...
        const romCache = (perGame && perGame.ke && typeof perGame.ke.romCache === 'boolean')
          ? perGame.ke.romCache
          : ((globalKe && typeof globalKe.romCache === 'boolean') ? globalKe.romCache : true);
      ...
      if (selectedEmulator === 'libretro') {
        ...
        if (romCache) {
          const prep = await libretroAdapter.prepareGame({ gameFilePath, utils: { ensureCachedJar } });
          preparedGamePath = prep?.preparedGamePath || gameFilePath;
        }
        console.log('[launch][libretro] using game path:', preparedGamePath);

        const { command, args, cwd } = libretroAdapter.buildCommand({ retroarchPath, corePath, gameFilePath: preparedGamePath });
        // Avoid pre-quoting; let buildCommandLine handle all quoting to prevent embedded quotes
      ...
      const extendedDefaults = {
        // core
        fullscreen: 0, width: 240, height: 320, scale: 2, keyLayout: 0, framerate: 60,
        // compat flags
        compatfantasyzonefix: 'off', compatimmediaterepaints: 'off', compatoverrideplatchecks: 'on', compatsiemensfriendlydrawing: 'off', compattranstooriginonreset: 'off',
        // extras
        backlightcolor: 'Disabled', fontoffset: '-2', rotate: '0', fpshack: 'Disabled', sound: 'on', spdhacknoalpha: 'off',
      };
      const baseParams = (perGame && perGame.freej2mePlus && perGame.freej2mePlus.useGlobal === false)
      ...
      const romCacheFree = (perGame && perGame.freej2mePlus && typeof perGame.freej2mePlus.romCache === 'boolean')
        ? perGame.freej2mePlus.romCache
        : ((globalFree && typeof globalFree.romCache === 'boolean') ? globalFree.romCache : true);
      const utilsForFree = {
        updateGameConf, DataStore, getConfigGameName,
