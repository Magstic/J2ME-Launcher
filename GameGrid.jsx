...
import useControllerContextMenu from './controller/useControllerContextMenu';
...
function GameGrid({ games, isLoading, onGameLaunch, onAddToFolder, onGameInfo, controllerModeEnabled = false, onToggleViewMode }) {
  ...
  const handlePress = (action) => {
    ...
    if (action === 'launch' && targetIndex >= 0 && games[targetIndex]) {
      ...
      const container = gridRef.current?.closest('.content-area');
      if (container) {
  ...
  return (
    <div className={`game-grid ${controllerModeEnabled ? 'controller-mode' : ''}`} ref={gridRef}>
